...
| `brand_id` | FK → brand، NULL | **SET_NULL**: حذف الماركة لا يحذف منتجاتها |
| `name` | VarChar(200) | اسم المنتج |
| `slug` | Slug(220) | **فريد**، تلقائي من الاسم، يدعم العربية |
| `sku` | VarChar(40) | رمز الجرد الداخلي (اختياري) — يدخل نص البحث |
...
| `stock` | PositiveInt | الكمية المتوفرة. **للمنتج ذي المقاسات/الألوان يُحسب تلقائياً** = مجموع مخزون متغيّراته المفعّلة (`sync_stock`)، ويُقفل بلوحة التحكم |
| `is_active` | Boolean | غير المفعّل لا يظهر بالمتجر إطلاقاً |
| `specs` | JSON | مواصفات **ثابتة للعرض فقط** (المنشأ، الكفالة). ما يختاره الزبون ويؤثر على المخزون (مقاس/لون) مكانه جداول الخيارات أدناه |
| `search_text` | Text | **نسخة مطبَّعة** من الاسم + الوصف + الماركة + الرمز للبحث العربي (سماعه ← سماعة، اصلي ← أصلي). تُحدَّث تلقائياً عند كل حفظ — لا تُعدَّل يدوياً |
| `created_at` / `updated_at` | DateTime | تلقائي |
...
### 3‑ب) `catalog_brand` — الماركات

| الحقل | النوع | ملاحظات |
|---|---|---|
| `id` | BigAuto | مفتاح أساسي |
| `name` | VarChar(100) | **فريد** — «أديداس»، «سامسونج» |
| `slug` | Slug(120) | **فريد**، تلقائي، يدعم العربية |
| `is_active` | Boolean | |

### 3‑ج) الخيارات والمتغيّرات — المقاسات والألوان

ثلاثة جداول تحلّ أهم نقص كان بالكاتالوج: **مقاس وقياس ولون لكل منتج،
ولكل تركيبة مخزونها وسعرها**.

**`catalog_productoption`** — محور اختيار للمنتج:

| الحقل | النوع | ملاحظات |
|---|---|---|
| `product_id` | FK → product | **CASCADE** |
| `name` | VarChar(50) | «المقاس»، «اللون» — **فريد لكل منتج** |
| `sort_order` | SmallInt | ترتيب المحاور بصفحة المنتج |

> **محوران كحدّ أقصى للمنتج** (يُفرض في `clean`): يغطّي البيع بالتجزئة
> عملياً (مقاس × لون)، ويسمح بفرض تفرّد التركيبة على مستوى قاعدة البيانات.

**`catalog_productoptionvalue`** — قيمة ضمن المحور:

| الحقل | النوع | ملاحظات |
|---|---|---|
| `option_id` | FK → productoption | **CASCADE** |
| `value` | VarChar(50) | «L»، «أبيض» — **فريدة لكل محور** |
| `sort_order` | SmallInt | ترتيب منطقي (S ثم M ثم L) لا أبجدي |

**`catalog_productvariant`** — التركيبة القابلة للشراء:

| الحقل | النوع | ملاحظات |
|---|---|---|
| `product_id` | FK → product | **CASCADE** |
| `value_1_id` | FK → optionvalue | **PROTECT** — قيمة المحور الأول |
| `value_2_id` | FK → optionvalue، NULL | **PROTECT** — قيمة المحور الثاني (NULL لمحور واحد) |
| `sku` | VarChar(40) | رمز جرد للتركيبة |
| `price` | Decimal(12,0)، NULL | **فارغ = سعر المنتج**. يُملأ فقط إذا هذا المقاس بسعر مختلف |
| `stock` | PositiveInt | **المخزون الحقيقي** — منه يُخصم عند الطلب |
| `is_active` | Boolean | إخفاء تركيبة دون حذف سجلّها |

**قيدا تفرّد** (لأن NULL لا يساوي NULL في SQL):
`(product, value_1, value_2)` حين `value_2` غير فارغ، و`(product, value_1)`
حين يكون فارغاً — فمستحيل تتكرّر نفس التركيبة.

**سلامة إضافية في `clean()`:** القيمة لازم تنتمي لخيارات هذا المنتج، وممنوع
قيمتان من نفس المحور (مقاسان معاً)، ولازم قيمة ثانية إذا للمنتج محوران.

## قرارات تصميم مهمّة (ولماذا)

0. **مخزون المنتج مُزامَن لا مكرَّر:** المخزون الحقيقي للمنتج ذي المقاسات
   موجود على التركيبات، لكن `Product.stock` يُعاد جمعه تلقائياً عند أي تغيير
   عليها. هكذا تبقى **كل** الاستعلامات والفلاتر والتقارير القديمة
   (`stock__gt=0`، فلتر «المتوفر»، تنبيه المخزون المنخفض) تعمل بلا تعديل،
   بدل نثر شروط مركّبة بكل استعلام. الحقل مقفول باللوحة ويُصحَّح تلقائياً
   لو عُدِّل يدوياً — فلا يفترق الرقم عن الحقيقة.
...
| `variant_id` | FK → productvariant، NULL | **PROTECT**: التركيبة المطلوبة (مقاس/لون). NULL لمنتج بلا خيارات |
| `product_name` | VarChar(200) | **لقطة**: اسم المنتج وقت الطلب |
| `variant_label` | VarChar(120) | **لقطة**: «المقاس: L، اللون: أبيض» — تبقى مقروءة حتى لو أُعيدت تسمية المقاسات لاحقاً |
| `unit_price` | Decimal(12,0) | **لقطة**: سعر القطعة وقت الطلب (سعر التركيبة الخاص إن وُجد) |
